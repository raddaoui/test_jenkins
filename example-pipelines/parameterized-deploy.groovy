...
pipeline {
    ...
    parameters {
        /* When changes are made to pipeline code which would modify UI elements in the job configuration page,
           these changes are not visible until the job is run at least once - initial setup is an example of this. By running
           the job manually with this default value, the deployment will be aborted without performing any real actions such that
           the UI updates.

           Uncheck this box when triggering manually to allow the job to run. */
